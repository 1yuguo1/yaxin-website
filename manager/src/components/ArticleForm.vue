...
const emits = defineEmits(['cnacel','add'])
...
const submit = (enum_) => {
    isResult.value = true
    const data = {'title':title.value,'content':content.value,'author':'玉溪亚信税务师事务所','category_id':props.message,'tags':'-','is_published':enum_}
    http.post('/carticle',data).then((res)=>{
